...
detectors.py
============
Two edge detectors + two corner finders, modular and mix-and-match.
...
-------------
    import cv2
    from detectors import StepDiff, Derivative, SimpleCorners, DenseCorners

    img  = cv2.imread("image.jpg")
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)

    # Detector A: step-difference + dense corners
    sd = StepDiff(gray, threshold=30, step=2)
    sd.detect()
    corners = DenseCorners(sd.ys, sd.centers, gray.shape).find()

    # Detector B: derivative + simple corners
    dv = Derivative(gray, threshold=15, step=2)
    dv.detect()
    corners = SimpleCorners(dv.ys, dv.centers).find()

    # Warp + draw
    warped = sd.warp(corners, gray)
    cv2.imshow("result", sd.draw(circle_r=3))
"""
